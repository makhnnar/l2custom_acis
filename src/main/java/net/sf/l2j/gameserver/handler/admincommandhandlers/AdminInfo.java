...
public class AdminInfo implements IAdminCommandHandler
{
	...
	private static void sendAiInfos(Npc npc, NpcHtmlMessage html)
	{
		...
		else
		{
			...
			for (Entry<ScriptEventType, List<Quest>> entry : npc.getTemplate().getEventQuests().entrySet())
			{
	...
	private static void sendSkillInfos(Npc npc, NpcHtmlMessage html)
	{
		...
		for (Entry<NpcSkillType, List<L2Skill>> entry : npc.getTemplate().getSkills().entrySet())
		{
			if (type != entry.getKey())
			{
				type = entry.getKey();
